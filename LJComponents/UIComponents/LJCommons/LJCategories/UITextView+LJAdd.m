...
@implementation UITextView (LJAdd)
...
{
    // 为placeholder赋值
    [self placeholderView].text = lj_placeholder;
}
...
- (void)setLj_placeholderColor:(UIColor *)lj_placeholderColor
{
    ...
    if (!self.placeholderExist) {
        ...
        self.placeholderView.textColor = lj_placeholderColor;
        
        // 动态添加属性的本质是: 让对象的某个属性与值产生关联
        objc_setAssociatedObject(self, PlaceholderColorKey, lj_placeholderColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
...
- (void)setLj_maxHeight:(CGFloat)lj_maxHeight
{
    CGFloat max = lj_maxHeight;
    
    // 如果传入的最大高度小于textView本身的高度，则让最大高度等于本身高度
    if (lj_maxHeight < self.frame.size.height) {
...
- (CGFloat)lj_maxHeight
{
    return [objc_getAssociatedObject(self, TextViewMaxHeightKey) doubleValue];
}

- (void)setLj_minHeight:(CGFloat)lj_minHeight
{
    objc_setAssociatedObject(self, TextViewMinHeightKey, [NSString stringWithFormat:@"%lf", lj_minHeight], OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (CGFloat)lj_minHeight
{
    return [objc_getAssociatedObject(self, TextViewMinHeightKey) doubleValue];
}

- (void)setLj_textViewHeightDidChanged:(textViewHeightDidChangedBlock)lj_textViewHeightDidChanged
{
    objc_setAssociatedObject(self, TextViewHeightDidChangedBlockKey, lj_textViewHeightDidChanged, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (textViewHeightDidChangedBlock)lj_textViewHeightDidChanged
{
...
- (void)autoHeightWithMaxHeight:(CGFloat)maxHeight textViewHeightDidChanged:(textViewHeightDidChangedBlock)textViewHeightDidChanged
{
    ...
    self.lj_maxHeight = maxHeight;
    if (textViewHeightDidChanged) self.lj_textViewHeightDidChanged = textViewHeightDidChanged;
}
...
- (void)refreshPlaceholderView {
    ...
    if (placeholderView) {
        ...
        if (self.lj_maxHeight < self.bounds.size.height) {
            self.lj_maxHeight = self.bounds.size.height;
        }
...
- (void)textViewTextChange {
    ...
    if (self.lj_maxHeight >= self.bounds.size.height) {
        ...
        if (currentHeight != self.lastHeight) {
            // 是否可以滚动
            self.scrollEnabled = currentHeight >= self.lj_maxHeight;
            CGFloat currentTextViewHeight = currentHeight >= self.lj_maxHeight ? self.lj_maxHeight : currentHeight;
            // 改变textView的高度
            if (currentTextViewHeight >= self.lj_minHeight) {
                CGRect frame = self.frame;
                frame.size.height = currentTextViewHeight;
                self.frame = frame;
                // 调用block
                if (self.lj_textViewHeightDidChanged) self.lj_textViewHeightDidChanged(currentTextViewHeight);
                // 记录当前高度
